...
**Quick recall:** the Preliminary Phase establishes the *capability to do architecture*: people (Architecture Board, domain architects), processes (governance, decision-making), tools (repository), and principles. Without this scaffolding in place, every subsequent phase improvises.
...
The Architecture Repository is the structured store for all architecture artifacts. See [Architecture Repository](../reference/architecture-repository.md) for the full structure.
...
**Judgment & trade-offs:** when establishing the practice:

| Question | Lean towards … when | Lean away when |
|---|---|---|
| **Mandating vs. advisory Board** | Architecture decisions are repeatedly bypassed; regulatory environment | Strong cultural respect for architecture exists already |
| **Heavy vs. light governance** | Many teams; high-risk; regulated | Single team; greenfield |
| **TOGAF strict vs. tailored** | Multi-business-unit enterprise; standardisation matters | Single team or product |
| **Many principles vs. few** | Mature practice; principles cited daily | New practice — start with 6–8, add as needed |

**Synthesis exercise:** draft a one-page "Architecture Practice Charter" for an organisation you know. State its scope, governance authority, the three most important principles, and how a junior architect would know what to do on day one. If anyone reading the charter would still be unsure, the charter is incomplete.
