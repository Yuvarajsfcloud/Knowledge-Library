...
**Judgment & trade-offs:** when classifying and tracing requirements:

| Question | Lean towards … when | Lean away when |
|---|---|---|
| **Heavy traceability matrix vs. light** | Regulated; safety-critical; complex multi-team | Single team; small scope |
| **All requirements managed centrally vs. distributed** | Compliance audit pressure | Domain teams own their slice; weekly sync sufficient |
| **Strict vs. flexible change process** | High delivery risk; integrated programme | Discovery-heavy; new product space |

**Synthesis exercise:** for one architecturally-significant requirement on a current project, walk the chain: Business need → Capability impacted → Architecture choice → Implementation evidence. Any broken link is unmanaged risk.
